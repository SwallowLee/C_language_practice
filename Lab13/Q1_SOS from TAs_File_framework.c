...
void save_data(gradeNode* myGrades)
{
    ...
    for(i=0;i<myGrades->studentNum;i++)
    {
        ...
        avg=avg+myGrades->studentList[i].grades;
    }
    avg=avg/myGrades->studentNum;
    fprintf(fp,"AVG             %.3f",avg);
    fclose(fp);
...
void load_data(gradeNode* myGrades)
{
    int i=0,grade=0;
    FILE *fp;
    char ch,filename[150],name[NAME_MAX_LENGTH],str[300];
    ...
    while(str[0]!='A'||str[1]!='V'||str[2]!='G')
    {
       ...
       //         printf("%c",str[i]);
        ...
        name[i]='\0';
        //printf("%s",name);
        strcpy(myGrades->studentList[myGrades->studentNum].name,name);
        while(str[i]==' ')
        {
            i++;
        }
        ...
        {
            if(str[i]>='0'&&str[i]<='9')
            grade=grade*10+(int )str[i]-'0';
            i++;
        }
        myGrades->studentList[myGrades->studentNum].grades=grade;
        myGrades->studentNum++;
        i=0;
        grade=0;
        fgets(str,sizeof(str),fp);
