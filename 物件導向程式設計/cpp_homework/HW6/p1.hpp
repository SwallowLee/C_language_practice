...
auto AVLTree<key_type, mapped_type, key_compare>::iterator::operator++(void) -> iterator& {
	...
	else
	{
		...
		CurrentNode=CurrentNode->Parent;//whether CurrentNode->Parent=nullptr
	}
...
auto AVLTree<key_type, mapped_type, key_compare>::Node::Find(Node* Tree, const key_type& KeyToFind, key_compare& Comp) -> Node* {
	...
	Node* CurrentNode = Tree;
    	...
    	{
        	CurrentNode = Find(&(CurrentNode--), CurrentNode, std::forward<key_type>(KeyToFind), Comp);
    	}
...
auto AVLTree<key_type, mapped_type, key_compare>::Node::RightRotate(Node* ThisNode) noexcept -> Node* {
       	...
       	Node* one=ThisNode->Left;
    	ThisNode->Left=one->Right;
    	if(one->Right!=nullptr)
    		one->Right->Parent=ThisNode;
    	one->Right=ThisNode;
	one->Parent=ThisNode->Parent;
    	ThisNode->Parent=one;
    	ThisNode->Height=(HeightOf(ThisNode->Left)>HeightOf(ThisNode->Right)?HeightOf(ThisNode->Left):HeightOf(ThisNode->Right))+1;
    	one->Height=(HeightOf(one->Left)>HeightOf(one->Right)?HeightOf(one->Left):HeightOf(one->Right))+1;
...
auto AVLTree<key_type, mapped_type, key_compare>::Node::LeftRotate(Node* ThisNode) noexcept -> Node* {
	Node* one=ThisNode->Right;
	ThisNode->Right=one->Left;
	if(one->Left!=nullptr)
		one->Left->Parent=ThisNode;
    	one->Left=ThisNode;
	one->Parent=ThisNode->Parent;
	ThisNode->Parent=one;
	ThisNode->Height=(HeightOf(ThisNode->Left)>HeightOf(ThisNode->Right)?HeightOf(ThisNode->Left):HeightOf(ThisNode->Right))+1;
